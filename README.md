        ...
        const storage = {
            ...
            delete: async (key) => {
                try {
                    ...
                    console.log('🗑️ Deleted from Firebase:', key);
                } catch (error) {
...
async loadPendingUsers() {
    try {
        ...
        console.log('📊 All users data from Firebase:', usersData);  // DEBUG LINE
        
        if (usersData) {
            this.pendingUsers = Object.values(usersData).filter(user => 
                user.status === 'pending' && user.role !== 'admin'
            );
            console.log('📋 Pending users filtered:', this.pendingUsers);  // DEBUG LINE
        } else {
            this.pendingUsers = [];
            console.log('⚠️ No users data found in Firebase');  // DEBUG LINE
        }
...
async approveUser(username) {
    ...
    try {
        ...
        console.log('🔍 Fetching user from Firebase path: users/' + username);
        ...
        if (!user) {
            ...
            if (!pendingUser) {
                alert('❌ User not found! The user may have been deleted or the username is incorrect.');
                console.error('❌ User not found at path: users/' + username);
                this.isSaving = false;
                return;
            }
            
            // Use the pending user data
            console.log('✅ Found user in pending users list');
            ...
            console.log('✏️ Updating user status to approved');
        ...
        alert('❌ Failed to approve user. Please try again. Error: ' + error.message);
    }
}
async rejectUser(username) {
    ...
    try {
        ...
        alert('❌ Failed to reject user. Please try again. Error: ' + error.message);
    }
}
calculateAdminStats() {
    const stats = {
        ...
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
            <div class="bg-white rounded-2xl p-4 sm:p-6 md:p-8 max-w-7xl w-full shadow-2xl my-8 max-h-[95vh] overflow-y-auto">
                <div class="flex justify-between items-center mb-6 sticky top-0 bg-white pb-4 border-b">
                    <div>
                        <h3 class="text-2xl sm:text-3xl font-bold text-gray-800">📊 Admin Dashboard</h3>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-4 text-white shadow-lg">
                        ...
                        <p class="text-3xl font-bold">GH₵${stats.totalSalesToday.toFixed(2)}</p>
                        <p class="text-xs mt-2 opacity-75">Cash: GH₵${stats.totalCashToday.toFixed(2)}</p>
                    </div>
                    
                    <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-4 text-white shadow-lg">
                        <p class="text-sm opacity-90">All-Time Sales</p>
                        <p class="text-3xl font-bold">GH₵${stats.totalSalesAllTime.toFixed(2)}</p>
                        <p class="text-xs mt-2 opacity-75">Cash: GH₵${stats.totalCashAllTime.toFixed(2)}</p>
                    </div>
                    ...
                    <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl p-4 text-white shadow-lg">
                        ...
                        <p class="text-3xl font-bold">GH₵${stats.totalDebt.toFixed(2)}</p>
                        <p class="text-xs mt-2 opacity-75">Outstanding payments</p>
                    </div>
                    
                    <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-4 text-white shadow-lg">
                        <p class="text-sm opacity-90">Inventory Value</p>
                        <p class="text-3xl font-bold">GH₵${stats.inventoryValue.toFixed(2)}</p>
                ...
                <div class="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl p-6 mb-6 border-2 border-indigo-200">
                    <h4 class="text-xl font-bold text-gray-800 mb-4">🏆 Top Selling Products</h4>
                                ...
                                <div class="flex justify-between items-start">
                                    <div class="flex-1">
                                        <div class="flex items-center gap-2">
                                            ${index < 3 ? `<span class="text-2xl">${index === 0 ? '🥇' : index === 1 ? '🥈' : '🥉'}</span>` : `<span class="font-bold text-gray-400">#${index + 1}</span>`}
                                        ...
                                        <p class="text-sm text-gray-600">Revenue: <span class="font-bold text-green-600">GH₵${data.revenue.toFixed(2)}</span></p>
                                    </div>
                                </div>
                            </div>
                        `)).join('')}
                ...
                ${stats.lowStockItems.length > 0 ? `
                    <div class="bg-gradient-to-r from-red-50 to-orange-50 rounded-xl p-6 mb-6 border-2 border-red-300">
                        <h4 class="text-xl font-bold text-red-800 mb-4">⚠️ Low Stock Alert (${stats.lowStockItems.length} items)</h4>
                ...
                <div class="bg-gray-50 rounded-xl p-6 mb-6">
                    <h4 class="text-xl font-bold text-gray-800 mb-4">📦 Inventory Overview: Products Sold vs Remaining</h4>
                ...
                <td class="p-3 text-purple-600 font-semibold">GH₵${remaining.totalValue.toFixed(2)}</td>
                <td class="p-3 text-green-600 font-bold">GH₵${sold.totalRevenue.toFixed(2)}</td>
              </tr>
                                    `;
                                }).join('')}
                            </tbody>
                ...
                <div class="bg-white rounded-xl p-6 border-2 border-gray-200">
                    <h4 class="text-xl font-bold text-gray-800 mb-4">📅 Daily Sales Breakdown</h4>
                    <div class="space-y-4">
                                ...
                                <div class="border-2 border-indigo-200 rounded-lg p-4 hover:shadow-lg transition">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
                                        <div class="bg-blue-50 rounded-lg p-3">
                                            ...
                                            <p class="text-xl font-bold text-blue-800">GH₵${dayData.totalSales.toFixed(2)}</p>
                                        </div>
                                        <div class="bg-green-50 rounded-lg p-3">
                                            <p class="text-xs text-green-600 font-medium">Cash Received</p>
                                            <p class="text-xl font-bold text-green-800">GH₵${dayData.totalCash.toFixed(2)}</p>
                                        </div>
                                        <div class="bg-red-50 rounded-lg p-3">
                                            <p class="text-xs text-red-600 font-medium">Outstanding</p>
                                            <p class="text-xl font-bold text-red-800">GH₵${dayData.totalDebt.toFixed(2)}</p>
                                        </div>
                                    </div>
                                        ...
                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                            ${Object.entries(dayData.itemsSold).map(([product, data]) => `
                                                <div class="flex justify-between text-sm">
                                                    ...
                                                    <span class="font-bold text-indigo-600">${data.quantity} units (GH₵${data.totalAmount.toFixed(2)})</span>
                                                </div>
                                            `).join('')}
                                        </div>
                                    </div>
    ...
    this.products.forEach(product => {
        if (product.addedBy) {
            activities.push({
                ...
                details: `${product.quantity} units at GH₵${product.retailPrice.toFixed(2)} retail price`,
    ...
    this.sales.forEach(sale => {
        if (sale.soldBy) {
            activities.push({
                ...
                details: `${sale.quantity} units for GH₵${sale.totalAmount.toFixed(2)}${sale.customerName ? ` to ${sale.customerName}` : ''}`,
                timestamp: sale.date,
                icon: '💰',
                color: 'green'
            });
        ...
        if (sale.recordedBy && sale.isOldSale) {
            activities.push({
                ...
                details: `${sale.quantity} units for GH₵${sale.totalAmount.toFixed(2)}`,
        ...
        if (sale.recordedBy && sale.isPaymentRecord) {
            activities.push({
                ...
                details: `GH₵${sale.amountPaid.toFixed(2)} - ${sale.paymentNote}`,
                timestamp: sale.date,
                icon: '💳',
                color: 'teal'
            });
...
this.activityLogs.forEach(log => {
    ...
    if (log.type === 'sale_deleted') {
        activities.push({
            ...
            details: `${log.quantity} units - GH₵${log.totalAmount.toFixed(2)} - Customer: ${log.customerName} - Reason: ${log.reason}`,
            timestamp: log.deletedAt,
            icon: '❌',
            color: 'red'
        });
    ...
    return `
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
            <div class="bg-white rounded-2xl p-4 sm:p-6 md:p-8 max-w-6xl w-full shadow-2xl my-8 max-h-[95vh] overflow-y-auto">
                <div class="flex justify-between items-center mb-6 sticky top-0 bg-white pb-4 border-b">
                    <div>
                        <h3 class="text-2xl sm:text-3xl font-bold text-gray-800">📋 Activity Log</h3>
...
async makeSale() {
    ...
    if (recentSales.length > 0) {
        const recentSaleDetails = recentSales.map((sale, index) => 
            `\n${index + 1}. ${sale.quantity} units - GH₵${sale.totalAmount.toFixed(2)} - ${new Date(sale.date).toLocaleTimeString()}${sale.customerName ? ` (Customer: ${sale.customerName})` : ''}`
        ).join('');

        const confirmDuplicate = confirm(
            `⚠️ DUPLICATE SALE WARNING\n\n` +
...
async deleteSale(sale) {
    ...
    const reason = prompt(`⚠️ IMPORTANT: Enter reason for deleting this sale:\n\nProduct: ${sale.productName}\nAmount: GH₵${sale.totalAmount.toFixed(2)}\nCustomer: ${sale.customerName || 'N/A'}\n\nReason:`);
        ...
        this.render();
            ...
            exportDailyToExcel(date, sales) {
                ...
                const summaryData = [
                    ...
                    ...Object.entries(summary.itemsSold).map(([product, data]) => ({
                        'Description': product,
                        'Value': `${data.quantity} units - GH₵${data.amount.toFixed(2)}`
                    })),
                    { 'Description': '', 'Value': '' },
                    { 'Description': 'FINANCIAL SUMMARY', 'Value': '' },
                    { 'Description': 'Total Items Sold', 'Value': summary.totalQuantity },
                    { 'Description': 'Total Sales Amount', 'Value': `GH₵${summary.totalAmount.toFixed(2)}` },
                    { 'Description': 'Debt Payments Received', 'Value': `GH₵${summary.paymentsReceived.toFixed(2)}` },
                    { 'Description': 'Total Cash Received', 'Value': `GH₵${summary.amountReceived.toFixed(2)}` },
                    { 'Description': 'Outstanding Balance', 'Value': `GH₵${summary.totalBalance.toFixed(2)}` }
                ];
            ...
            exportDailyToWord(date, sales) {
                ...
                sales.forEach(sale => {
                    ...
                    htmlContent += `
                        <tr class="${rowClass}">
                            <td>${sale.productName}${sale.isPaymentRecord ? `<br><small style="color: #059669; font-weight: bold;">💰 ${sale.paymentNote}</small>` : ''}</td>
                            <td style="${sale.customerName ? 'color: #4F46E5; font-weight: bold;' : 'color: #9CA3AF;'}">${sale.customerName || '-'}</td>
                            <td>${sale.quantity || '-'}</td>
                            <td>${sale.salePrice > 0 ? 'GH₵' + sale.salePrice.toFixed(2) : '-'}</td>
                            <td style="font-weight: bold;">${sale.isPaymentRecord ? '-' : 'GH₵' + sale.totalAmount.toFixed(2)}</td>
                            <td style="color: #059669; font-weight: bold;">GH₵${sale.amountPaid.toFixed(2)}</td>
                            <td style="${sale.balance > 0 ? 'color: #DC2626; font-weight: bold;' : 'color: #9CA3AF;'}">${sale.balance > 0 ? 'GH₵' + sale.balance.toFixed(2) : '-'}</td>
                ...
                Object.entries(summary.itemsSold).forEach(([product, data]) => {
                    htmlContent += `
                        <div class="summary-item">
                            ...
                            <span class="summary-value">${data.quantity} units - GH₵${data.amount.toFixed(2)}</span>
                        </div>
                    `;
                });
                ...
                htmlContent += `
                                ...
                                <div class="summary-box">
                                    ...
                                    <div class="summary-item">
                                        ...
                                        <span class="summary-value text-blue">GH₵${summary.totalAmount.toFixed(2)}</span>
                                    </div>
                `;
                ...
                if (summary.paymentsReceived > 0) {
                    htmlContent += `
                                    <div class="summary-item">
                                        ...
                                        <span class="summary-value text-orange">GH₵${summary.paymentsReceived.toFixed(2)}</span>
                                    </div>
                    `;
                }
                ...
                htmlContent += `
                                    <div class="summary-item">
                                        ...
                                        <span class="summary-value text-green">GH₵${summary.amountReceived.toFixed(2)}</span>
                                    </div>
                `;
                ...
                if (summary.totalBalance > 0) {
                    htmlContent += `
                                    <div class="summary-item">
                                        ...
                                        <span class="summary-value text-red">GH₵${summary.totalBalance.toFixed(2)}</span>
                                    </div>
                    `;
                }
                        ...
                        <div class="bg-white rounded-2xl shadow-xl p-4 sm:p-6 md:p-8">
                            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 sm:mb-8 gap-4">
                                <h1 class="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-800 flex items-center gap-2 sm:gap-3">
                                    ...
                                    <span class="text-xs bg-green-100 text-green-700 px-2 sm:px-3 py-1 rounded-full font-normal">🔥 Live</span>
                                </h1>
<div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full sm:w-auto">
                                    <div class="text-sm text-gray-600">
                                        <span class="font-semibold">👤 ${this.getUserDisplayName()}</span>
...
<button onclick="app.showDeleteButtons = !app.showDeleteButtons; app.render()" 
    ...
    ${this.showDeleteButtons ? '🔓 Hide Delete' : '🔒 Show Delete'}
</button>
...
>
    ${this.filterLowStock ? '✓ Low Stock' : 'Show Low Stock'}
</button>
<button 
...
>
    ${this.filterOutOfStock ? '✓ Out of Stock' : 'Show Out of Stock'}
</button>
        </div>
                            ...
                            ` : `
                                <div class="bg-purple-50 border-2 border-purple-200 rounded-lg p-4 mb-6">
                                    <p class="text-purple-800 font-semibold">
                                        📋 ${this.pendingUsers.length} user(s) pending approval
                                    </p>
                                </div>
                        ...
                        <div class="p-4 sm:p-6">
                            ...
                            <div class="grid grid-cols-2 gap-2 mb-4 text-sm">
                                <div class="bg-blue-50 p-2 sm:p-3 rounded-lg">
                                    ...
                                    <p class="font-bold text-blue-800">GH₵${product.distributorPrice.toFixed(2)}</p>
                                </div>
                                <div class="bg-green-50 p-2 sm:p-3 rounded-lg">
                                    <p class="text-xs text-green-600 font-medium">Retail Price</p>
                                    <p class="font-bold text-green-800">GH₵${product.retailPrice.toFixed(2)}</p>
                                </div>
                            </div>
                            ...
                            <div class="space-y-4">
                                ...
                                ${totalAmount > 0 ? `
                                    <div class="bg-blue-50 p-3 sm:p-4 rounded-lg border-2 border-blue-200">
                                        <p class="text-base sm:text-lg font-bold text-gray-800">Total Amount: GH₵${totalAmount}</p>
                                    </div>
                                ` : ''}
                                ...
                                ${balance > 0 ? `
                                    <div class="bg-yellow-50 p-3 sm:p-4 rounded-lg border-2 border-yellow-400">
                                        <p class="text-base sm:text-lg font-bold text-red-600">Balance Due: GH₵${balance}</p>
                                    </div>
                ...
                <div class="space-y-6">
                    <div class="bg-blue-50 border-2 border-blue-200 rounded-lg p-4">
                        <h4 class="font-bold text-blue-900 mb-2 flex items-center gap-2">
                            <span>ℹ️</span> How to Use Bulk Upload
                        </h4>
            ...
            renderUnpaidSales() {
                ...
                return `
                    <div class="bg-gradient-to-r from-yellow-50 to-orange-50 rounded-xl p-4 sm:p-6 mb-6 sm:mb-8 border-2 border-yellow-300">
                        <h2 class="text-xl sm:text-2xl font-bold mb-4 text-gray-800">⚠️ Unpaid Sales (${unpaidSales.length})</h2>
                                ...
                                <tbody class="bg-white">
                                    ${unpaidSales.map(sale => `
                                        <tr class="border-t hover:bg-yellow-50">
                                            ...
                                            <td class="p-2 sm:p-3 text-sm sm:text-base">GH₵${sale.totalAmount.toFixed(2)}</td>
                                            <td class="p-2 sm:p-3 text-green-600 text-sm sm:text-base">GH₵${sale.amountPaid.toFixed(2)}</td>
                                            <td class="p-2 sm:p-3 font-bold text-red-600 text-sm sm:text-base">GH₵${sale.balance.toFixed(2)}</td>
                    ...
                    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                        <div class="bg-white rounded-2xl p-4 sm:p-6 md:p-8 max-w-md w-full shadow-2xl">
                            ...
                            <div class="bg-yellow-50 p-3 sm:p-4 rounded-lg mb-4 border-2 border-yellow-300">
                                ...
                                <p class="text-xs sm:text-sm text-gray-600">Total: GH₵${this.selectedSaleForPayment.totalAmount.toFixed(2)}</p>
                                <p class="text-xs sm:text-sm text-green-600">Paid: GH₵${this.selectedSaleForPayment.amountPaid.toFixed(2)}</p>
                                <p class="text-sm sm:text-base text-red-600 font-bold">Balance Due: GH₵${this.selectedSaleForPayment.balance.toFixed(2)}</p>
                            </div>
...
renderSalesTable() {
    ...
    const sortedDates = Object.keys(salesByDay).sort((a, b) => {
        // Parse dates correctly for proper chronological sorting
...
` : ''}
    ...
    <td class="p-2 sm:p-3 text-xs sm:text-sm">${sale.salePrice > 0 ? 'GH₵' + sale.salePrice.toFixed(2) : '-'}</td>
    <td class="p-2 sm:p-3 font-semibold text-xs sm:text-sm">${sale.isPaymentRecord ? '-' : 'GH₵' + sale.totalAmount.toFixed(2)}</td>
    <td class="p-2 sm:p-3 text-green-600 font-semibold text-xs sm:text-sm">GH₵${sale.amountPaid.toFixed(2)}</td>
    <td class="p-2 sm:p-3 ${sale.balance > 0 ? 'font-bold text-red-600' : 'text-gray-400'} text-xs sm:text-sm">
        ${sale.balance > 0 ? 'GH₵' + sale.balance.toFixed(2) : '-'}
    </td>
                                                            ...
                                                            ${Object.entries(summary.itemsSold).map(([product, data]) => `
                                                                <div class="flex justify-between items-center bg-white p-2 sm:p-3 rounded-lg shadow-sm">
                                                                    ...
                                                                    <div class="text-right ml-2 flex-shrink-0">
                                                                        ...
                                                                        <div class="text-xs text-gray-600">GH₵${data.amount.toFixed(2)}</div>
                                                                    </div>
                                                                </div>
                                                            `).join('')}
                                                        </div>
                                                    ...
                                                    <div class="flex flex-col justify-center">
                                                        <div class="bg-white rounded-lg p-4 sm:p-6 shadow-md border-2 border-green-200 mb-4">
                                                            <div class="text-center">
                                                                ...
                                                                <p class="text-2xl sm:text-3xl md:text-4xl font-bold text-blue-600 mb-2 sm:mb-3">GH₵${summary.totalAmount.toFixed(2)}</p>
                                                                ...
                                                                ${summary.paymentsReceived > 0 ? `
                                                                    ...
                                                                    <p class="text-xl sm:text-2xl font-bold text-orange-600 mb-2 sm:mb-3">GH₵${summary.paymentsReceived.toFixed(2)}</p>
                                                                ` : ''}
                                                                ...
                                                                <p class="text-2xl sm:text-3xl font-bold text-green-600">GH₵${summary.amountReceived.toFixed(2)}</p>
                                                            </div>
                                                        </div>
                                                        ...
                                                        ${summary.totalBalance > 0 ? `
                                                            <div class="bg-gradient-to-r from-red-100 to-orange-100 rounded-lg p-4 sm:p-6 shadow-md border-2 border-red-300">
                                                                <div class="text-center">
                                                                    ...
                                                                    <p class="text-2xl sm:text-3xl md:text-4xl font-bold text-red-600">GH₵${summary.totalBalance.toFixed(2)}</p>
                                                                </div>
                                                            </div>
                                                        ` : ''}
                                                    </div>
